...
public class DatabaseWebSecurity {
    ...
    public UserDetailsManager customUsers(DataSource dataSource){
        JdcUserDetailsManager users = new JdbcDetailsManager(dataSource);
        users.setUserByUsernameQuery("select login, clave, status from usuarios where login = ?");
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        http.autorizedHttpRequests(authorize -> authorize
