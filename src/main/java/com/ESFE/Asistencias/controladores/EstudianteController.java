...
public class EstudianteController {
    ...
    public String index(Model model, @RequestParam("page") Optional<Integer> page, @RequestParam("size") Optional<Integer> size) {
        ...
        return "estudiante/index"; // Asegúrate de que la vista existe en src/main/resources/templates/estudiante/index.html
    }

    @GetMapping("/create")
    public String createForm(Model model) {
        model.addAttribute("estudiante", new Estudiante());
        return "estudiante/create"; // Asegúrate de que la vista existe en src/main/resources/templates/estudiante/create.html
    }

    @PostMapping("/save")
    public String save(Estudiante estudiante, BindingResult result, Model model, RedirectAttributes attributes) {
        if (result.hasErrors()) {
            model.addAttribute("estudiante", estudiante); // Añadir el estudiante al modelo en caso de error
            attributes.addFlashAttribute("error", "No se pudo guardar debido a un error.");
            return "estudiante/create"; // Redirige a la vista de creación
        }
